...
jdata_1=jdata_1.sample(frac=1)
jdata_2=jdata_2.sample(frac=1)
captions = []
dataImages = []
# print(images)
vocab = get_vocab('vocab.pkl')
...
# print('classes',jdata_1['Problems'].value_counts()+jdata_2['Problems'].value_counts())
...
actor = models_modified.Actor(args.vocab_size,
                    args.dec_hsz,
                    args.rnn_layers,
                    args.batch_size,
                    args.max_len,
...
critic = models_modified.Critic(args.vocab_size,
                      args.dec_hsz,
                      args.rnn_layers,
                      args.batch_size,
                      args.max_len,
...
EncoderDecoder = models_modified.EncDec(args.dec_hsz, args.vocab_size).cuda()
...
def pre_train_actor(epoch):
    ...
    for plain_img_paths,plain_imgs,imgs, labels in tqdm(training_data,
        ...
        # enc=actor.encode(plain_imgs) #layer
        # hidden = actor.feed_enc(enc)
        # target = actor(hidden, labels)
    
        outputs,words = actor(plain_img_paths,plain_imgs,labels)
        # print('actor parameters',list(actor.named_parameters()))
        # print('word_out_shape',words.shape)
        # print('target_shape',target.shape)
        # print('labels_shape',labels.shape)
        # print('labels.view',labels.view(-1).shape)
        # print('target.view',target.view(-1, target.size(2)).shape)
        loss = criterion_A(outputs.view(-1, outputs.size(2)), labels.view(-1))
        loss.backward()
...
def train_actor_critic(GAMMA, epoch):
    ...
    for plain_img_paths,plain_imgs,imgs, labels in tqdm(training_data,
        ...
        enc = actor.encode(plain_imgs)
        # hidden_A = actor.feed_enc(enc)
        target,words=actor(plain_img_paths,plain_imgs,labels)
        words=torch.LongTensor(words)
        words=words.view(1,-1).to(device)
        ...
        print("LOSS", loss_a.item(),file=open(args.train_logs,'a'))
