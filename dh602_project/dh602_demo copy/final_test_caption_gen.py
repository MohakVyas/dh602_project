...
parser = argparse.ArgumentParser(description='Actor Dual-Critic Image Captioning')
parser.add_argument('--model_path',type=str,required=True)
parser.add_argument('--img_path',type=str,required=True)
parser.add_argument('--is_lstm',type=str,required=True)
...
# print(actor_m,file=open('./logs.txt','a'))
...
if args.is_lstm=="True":
    print('loading lstm')
    actor.load_state_dict(checkpoint['model'])
    actor.to(device)
    print(actor,file=open('./logs.txt','a'))
else:
    print('loading transformer')
    actor_m.load_state_dict(checkpoint['model'])
    actor_m.to(device)
    print(actor_m,file=open('./logs.txt','a'))
...
def testSentences_transformer(dataImages):
    ...
    for plain_img_paths,plain_imgs,imgs, labels in tqdm(training_data,
        ...
        idx+=1
        print('image',plain_img_paths,file=open('./logs.txt','a'))
        print('predicted:',predicted,file=open('./logs.txt','a'))
        print('ground truth:',truth,file=open('./logs.txt','a'))
...
def testSentences(dataImages):
    ...
    listTrue = []
    idx=0
    for plain_img_paths,plain_imgs,imgs, labels in training_data:
...
if args.is_lstm=='True': 
    testSentences(args.img_path)
else:
    print('transformer')
    testSentences_transformer(args.img_path)
...
#         img.save(img_path) 
#         img_tokens = testSentences(img_path)
