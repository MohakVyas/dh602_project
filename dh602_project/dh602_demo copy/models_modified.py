...
VOCAB_SIZE=1544 #found using vocab_build.py for findings part
...
caption_model=ImageCaptioningModel(cnn_model=model_swin,encoder=encoder,decoder=decoder)
...
class Actor(nn.Module):
    ...
    def forward(self,image_paths, images,captions,labels=None):
        ...
        # else:
        #     print('words_shape',torch.cat(words,1).shape)

        #     return torch.cat(outputs, 1), torch.cat(words, 1)
        # images=images.to(device)
        # captions=captions.to(device)
        outputs=self.caption_model(images,captions,True).to(device)
        pad_tensor=torch.zeros(self.bsz,1,1544).to(device)
        # print('pad_tensor_shape',pad_tensor.shape)
        # print('outputs_shape',outputs.shape)
        outputs=torch.cat((outputs,pad_tensor),dim=1).to(device)
        # print('outputs shape',outputs.shape)
        props = F.log_softmax(outputs, dim=-1)
        # print('props shape',props.shape)
        # print('outputs',outputs)
        _props = props.data.clone().exp()
        words=[]
        for i in range(props.shape[1]):
            word = Variable(_props[:,i,:].multinomial(1), requires_grad=False)
            words.append(word)
        # print('words shape',len(words))
        # output_data=Data_loader(image_paths,outputs,60,images.shape[0],is_cuda=True,img_size=32)
